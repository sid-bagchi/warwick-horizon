...
PDR = 2:0.01:3; % port dia ratio Df/Di, Ch. 7, pp. 134
D_f = 60e-3; % final fuel grain port dia, m
D_i = D_f./PDR; % initial fuel grain port dia, m
L_f = 100e-3; % fuel length, m
m_f = 0; % total fuel mass, kg
p_f = 900; % fuel density, kg m-3
m_o = 0.5:0.02:2.5; % total oxidiser mass, kg - GUESS
...
t_b = (((D_f^(2*n+1) - D_i.^(2*n+1)) .* pi^n) ./ (a_o * (2*n + 1) * 2^(2*n + 1) * m_o .^ n)) .^ (1 / (1-n)); % Check! Unrealistic values
 

plot(m_o./m_f, t_b);
xlabel('O/F for N2O and Paraffin'); ylabel('burn time');